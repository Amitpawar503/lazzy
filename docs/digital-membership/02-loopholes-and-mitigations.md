...
- **C4 — Atomic single redemption:** redemption is a single conditional DB write, unique per `(entitlement, checkpoint)`; concurrent scans → exactly one winner.
...
- **C9 — Whitelist + OTP:** only pre-approved `(event, mobile)` pairs can log in; OTP proves possession; single active session per number.
...
- **Stops it:** **C4** — the first scan redeems `(entitlement, ENTRY)`; the second returns **`ALREADY_USED`** with the first-claim timestamp. Only **one** admission happens. This is the exact "duplicate customer, first scan allowed" behavior Tushar described.
- **Residual risk:** if the **impostor scans first**, the real customer is the one who gets `ALREADY_USED`. The system prevents *double* entry but not *wrong-person* entry within a live window.
  - **Mitigations:** (a) staff see a **masked holder identity** on the result (Q14) to eyeball the person; (b) `ALREADY_USED` shows first-claim time so a wronged genuine customer is a clean dispute (scan-history API, FR35); (c) short TTL shrinks the window. Accept as a known residual — true elimination needs live customer↔device binding, which is out of scope.

## 3. Concurrent double-scan on two gates/devices (race condition)
- **Stops it:** **C4** implemented as an atomic conditional write / unique constraint on `(entitlement_id, checkpoint)` (see LLD). Two simultaneous `UPDATE … WHERE status='UNUSED'` → exactly one affects a row (`ENTRY_ALLOWED`), the other affects zero (`ALREADY_USED`). No lost update, no double admit. Directly satisfies FR25.
...
- **Stops it:** **C7** — the QR carries no event; the event comes from the staff **session**. Backend resolves entitlement for `(session.eventId, tokenCustomer)`. No Event-A entitlement exists for Event B → **`NOT_ENTITLED`**. Satisfies FR31.
...
- **Stops it:** **C9** — login requires a whitelisted `(eventId, mobile)` **and** OTP possession; **no OTP is ever initiated** for a non-whitelisted pair (also prevents number/event enumeration — generic messaging, Q11). Single active session per number (FR21) limits sharing; a fresh login elsewhere invalidates the old session (`STAFF_SESSION_INVALID`).
- **Residual:** a whitelisted staffer can still hand their logged-in phone to someone. Bounded by session TTL (Q13), audit trail (every scan carries staff id), and checkpoint-scoping.
...
## 12. OTP brute force / SMS bombing on the login endpoint
- **Stops it:** **C9** + OTP attempt limits, expiry, single-use, resend cooldown + per-number/per-IP rate limits (Q10). Non-whitelisted pairs never trigger an SMS at all.

## 13. Winner list not loaded (or partially loaded) at event start
- **Stops it:** **default-deny** — no entitlement row ⇒ `NOT_ENTITLED`. The system fails safe (no accidental admits), never fails open. Ops mitigations: a pre-event **readiness check** (list loaded? counts match?) and support for **mid-event appends** (Q15).
...
|---|---|
| Non-winner → `NOT_ENTITLED`, not `INVALID_QR` | C7 + entitlement lookup |
| One event's win not admissible at another | C7 |
| First scan `ENTRY_ALLOWED` + atomic mark; subsequent `ALREADY_USED` | C4, C6 |
...
| Only whitelisted `(event, mobile)` gets OTP; new login kills old session | C9 |
